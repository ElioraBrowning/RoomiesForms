  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="card">
      ...
      {fields.map((field, index) => (
        <div key={field.id} style={{ display: 'flex', gap: '1rem', marginBottom: '1rem', alignItems: 'flex-start', padding: '1rem', border: '1px solid #eee', borderRadius: '4px' }}>
      ...
      {!isReadonly && (
        <button type="button" className="btn" onClick={() => append({ week: fields.length + 1, description: '', hours: 0 })} style={{ marginBottom: '1rem', backgroundColor: '#6c757d' }}>
          + Add Week
        </button>
      )}
